// ============================================================
// products-load.js
// Dynamically loads products for ALL category pages.
// Works for: anime, games, kpop, movies, series, music, pop
//
// HOW IT WORKS:
// Each category HTML has a container like:
//   <div id="productContainer" data-category="anime"></div>
// This script reads the data-category attribute and fetches
// only that category's products from the backend.
// ============================================================
...
const BASE_URL = window.API_BASE || "http://127.0.0.1:5000";

// ── Category page grid ──────────────────────────────────────
// Each category page has a different CSS class for its grid items.
// Map category → masonry item class so clicks open the modal correctly.
const CATEGORY_CLASS_MAP = {
    anime:   "masonry-item-category6",
    games:   "masonry-item-category7",
    kpop:    "masonry-item-category2",
    music:   "masonry-item-category1",
    pop:     "masonry-item-category3",
    movies:  "masonry-item-category5",
    series:  "masonry-item-category5",
    "movies&series": "masonry-item-category4",
};
...
if (container) {
    ...
    const itemClass = CATEGORY_CLASS_MAP[category] || "masonry-item-category6";
    ...
    fetch(`${BASE_URL}/api/products?category=${category}`)
        ...
        .then(products => {
            ...
            if (products.length === 0) {
                container.innerHTML = `<p style="text-align:center; padding:40px; opacity:0.6;">No products found in this category.</p>`;
                return;
            }
            ...
            products.forEach(product => {
                ...
                card.innerHTML = `
                    ...
                    <div class="product-info">
                        <h4>${product.name}</h4>
                        <p class="price1">₹${product.price.toLocaleString("en-IN")}</p>
                        <p>List Price: <del>₹${product.old_price.toLocaleString("en-IN")}</del></p>
                        <p class="rating">⭐ ${product.rating}</p>
                    </div>
                `;

                // Click on card → open modal with full product data
        ...
        .catch(error => {
            ...
            container.innerHTML = `<p style="text-align:center; padding:40px; color:red;">Could not load products. Make sure the backend server is running.</p>`;
        });
}


// ============================================================
// PRODUCT MODAL  (shared across all pages)
// Opens the modal and fills in all product details dynamically.
// ============================================================
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const modal   = document.getElementById("productModal");
     const closeBtn = document.querySelector(".close-btn") || document.querySelector(".close-btn-model");
...
async function addToCart(productId, productName) {
    ...
    try {
        ...
        if (!res.ok) {
            showToastMsg(data.error || "Failed to add item to cart");
            return;
        }
        ...
        showToastMsg("Could not reach server. Is backend running?");
    }
}
