// Anime page: product modal behavior.

const modal = document.getElementById("productModal");
const closeBtn = document.querySelector(".close-btn");

if (modal && closeBtn) {
  document.querySelectorAll(".masonry-item-category6 img").forEach((img) => {
    img.addEventListener("click", function () {
      ...
      if (modalTitle)
        modalTitle.textContent = "Agust D Artist Edition Stainless Steel Mug";
      if (modalDescription)
        modalDescription.textContent =
          "Premium anime inspired insulated travel mug. Perfect for fans.";
      if (modalPrice) modalPrice.textContent = "₹1,299";
      if (modalListPrice) modalListPrice.textContent = " List Price: ₹1,999.00";
      if (modalSave) modalSave.textContent = "You Save: ₹700 (35% OFF)";
      if (modalRating)
        modalRating.textContent = "⭐ 4.7 out of 5 stars (1,240 ratings)";
