// Kpop page: product modal behavior (category2 grid).

const kpopModal = document.getElementById("productModal");
const kpopCloseBtn = document.querySelector(".close-btn");

if (kpopModal && kpopCloseBtn) {
  document
    .querySelectorAll(".masonry-item-category2 img")
    .forEach((img) => {
      img.addEventListener("click", function () {
        ...
        kpopModal.classList.add("active");
      });
    });

  kpopCloseBtn.addEventListener("click", () => {
    kpopModal.classList.remove("active");
  });

  window.addEventListener("click", (e) => {
    if (e.target === kpopModal) {
      kpopModal.classList.remove("active");
    }
  });
}
